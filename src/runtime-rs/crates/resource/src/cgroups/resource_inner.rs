...
impl CgroupsResourceInner {
    fn new_cgroups(config: &CgroupConfig) -> Result<(CgroupManager, Option<CgroupManager>)> {
    ...
    pub(crate) fn new(config: &CgroupConfig) -> Result<Self> {
        let (mut sandbox_cgroup, mut overhead_cgroup) =
            Self::new_cgroups(config).context("create new cgroups")?;

        // The runtime is prioritized to be added to the overhead cgroup.
        // If not exists, add it to the sandbox cgroup.
    ...
    pub(crate) fn restore(config: &CgroupConfig) -> Result<Self> {
        let (sandbox_cgroup, overhead_cgroup) =
            Self::new_cgroups(config).context("restore cgroups")?;
...
impl CgroupsResourceInner {
    ...
    async fn move_vcpus_to_sandbox_cgroup(&mut self, hypervisor: &dyn Hypervisor) -> Result<usize> {
        ...
        if !self.sandbox_cgroup.systemd() && !self.sandbox_cgroup.v2() {
            ...
            let tgid =
                get_tgid_from_pid(vcpu as i32).context("get tgid from hypervisor vmm pid")? as u64;
            self.sandbox_cgroup
    ...
    async fn update_sandbox_cgroups(&mut self, hypervisor: &dyn Hypervisor) -> Result<bool> {
        // The runtime is under overhead cgroup if available, so we have to
        // move VMM process to the sandbox cgroup. We also need to move
        // first before setting the resources, since cgroup managers create
        // cgroups lazily If no process is ever added to it.
        if self.overhead_cgroup.is_some() {
            let vcpu_num = self
                .move_vcpus_to_sandbox_cgroup(hypervisor)
                .await
                .context("move vcpus to sandbox cgroup")?;
            // The hypervisor might not be started yet. If that's the case,
            // we don't have to set the resources at this time.
            if vcpu_num == 0 {
                return Ok(false);
            }
        }
...
impl CgroupsResourceInner {
        ...
        let old = match op {
            ResourceUpdateOp::Add | ResourceUpdateOp::Update => {
                let resources = resources.ok_or_else(|| {
                    anyhow::anyhow!("resources should not be None for Add or Update operation")
                })?;
    ...
    pub(crate) async fn setup_after_start_vm(&mut self, hypervisor: &dyn Hypervisor) -> Result<()> {
        ...
        // In the case where overhead cgroup is enabled and we can't move
        // the hypervisor vCPUs to the sandbox cgroup, the hypervisor is in
        // overhead cgroup. It means the hypervisor is not controlled by
        // cgroups at all, which is not a desirable outcome for us.
        //
        // The main reason is that the hypervisor cannot provide vCPUs'
        // thread id. If you are working with that hypervisors, you should
        // either enable `sandbox_cgroup_only` or ensure that
        // `Hypervisor::get_thread_ids()` functions properly.
...
/// Recreate a Linux resources containing cpuset.cpus and cpuset.mems only.
